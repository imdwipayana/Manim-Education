...
class randomLineMeasurement(Scene):
    def construct(self):
        # Title
        title = Text("Measuring Length \n with Random Number", font_size=48, weight=BOLD, gradient=(BLUE, GREEN), line_spacing=1.2)
        self.play(Write(title))
        self.play(title.animate.to_edge(UP))
        self.wait(1)

        # The line to be measured (6cm)
        true_length = 6
        ruler_length = 10
        ...
        line_label = Text("Line to be measured (? cm)", font_size=32, color=RED)
        ...
        ruler = Line(
            ...
            color=YELLOW, stroke_width=6).shift(DOWN*0.5)
        ...
        ruler_label = Text("10 cm ruler", font_size=32, color=YELLOW)
        ruler_label.next_to(ruler, DOWN, buff=0.5)

        self.play(Create(ruler), Create(ruler_marks), Write(ruler_numbers), Write(ruler_label))
        self.wait(1)

        # Explanation
        explanation =  Text("Generate random number between 0 and 10 \n Count how many fall between the line",
                            font_size = 28,
                            line_spacing = 1.2,
        ...
        sim_line = Line( 
            ...
            color=RED, stroke_width=10).shift(UP*2.5)
        
        # ruler scale on top
        ruler_scale = Line(
            LEFT*line_width/2,
            RIGHT*line_width/2,
            color=YELLOW, stroke_width=6).shift(UP*3)
